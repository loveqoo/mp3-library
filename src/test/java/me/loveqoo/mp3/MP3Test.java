...
public class MP3Test {
	...
	public void findHead() {
		...
		Function<FileChannel, Optional<Long>> WRAPPED_HEADER_POS = HOLD_POSITION(GET_HEADER_POSITION);
		Function<FileChannel, Optional<byte[]>> WRAPPED_HEADER_RAW = HOLD_POSITION(GET_HEADER_RAW);

		WithFile.work(filePath, HOLD_POSITION((ch) -> {
			Optional<Long> headerPos = WRAPPED_HEADER_POS.apply(ch);
			assert (headerPos.isPresent());
			SET_CHANNEL_POSITION.accept(ch, headerPos.get());
