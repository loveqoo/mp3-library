...
import qoo.lib.audio.infrastructure.Bytes;
import qoo.lib.audio.infrastructure.InvalidResultException;
import qoo.lib.audio.infrastructure.ThreadSafe;
...
import static qoo.lib.audio.infrastructure.Bytes.*;
...
public class MP3 {
    ...
    static public enum MP3_TYPE {
        CBR, VBR_XING_X, VBR_XING_I, VBR_VBRI;
    }

    static public byte[] getVbrIdentifier(MP3_TYPE vbrType) {
        if (vbrType == MP3_TYPE.VBR_XING_X) {
            return VBR_IDENTIFIRE[0];
        } else if (vbrType == MP3_TYPE.VBR_XING_I) {
    ...
    static public byte[] findVbrRawToc(byte[] vbrHeader) {
        MP3_TYPE vbrType = findVbrType(vbrHeader);
        if (vbrType == MP3_TYPE.VBR_VBRI) {
